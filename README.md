...
- AVS contracts
  - [ServiceManager](contracts/src/ICrossMessagingServiceManager.sol) which will eventually contain slashing logic but for M2 is just a placeholder.
  - [TaskManager](contracts/src/CrossMessagingTaskManager.sol) which contains [task creation](contracts/src/CrossMessagingTaskManager.sol#L83) and [task response](contracts/src/CrossMessagingTaskManager.sol#L102) logic.
  - The [challenge](contracts/src/CrossMessagingTaskManager.sol#L176) logic could be separated into its own contract, but we have decided to include it in the TaskManager for this simple task.
...
1. A task generator (in our case, same as the aggregator) publishes tasks once every regular interval (say 10 blocks, you are free to set your own interval) to the CrossMessagingTaskManager contract's [createNewTask](contracts/src/CrossMessagingTaskManager.sol#L83) function. Each task specifies an integer `crossChainMessage` for which it wants the currently opted-in operators to determine its square `crossChainMessage^2`. `createNewTask` also takes `quorumNumbers` and `quorumThresholdPercentage` which requests that each listed quorum (we only use quorumNumber 0 in incredible-squaring) needs to reach at least thresholdPercentage of operator signatures.
...
4. [Aggregator] The aggregator collects the signatures from the operators and aggregates them using BLS aggregation. If any response passes the [quorumThresholdPercentage](contracts/src/ICrossMessagingTaskManager.sol#L36) set by the task generator when posting the task, the aggregator posts the aggregated response to the Task contract.

5. If a response was sent within the [response window](contracts/src/CrossMessagingTaskManager.sol#L119), we enter the [Dispute resolution] period.
   - [Off-chain] A challenge window is launched during which anyone can [raise a dispute](contracts/src/CrossMessagingTaskManager.sol#L171) in a DisputeResolution contract (in our case, this is the same as the TaskManager contract)
