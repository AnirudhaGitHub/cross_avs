...
func NewAvsManagersBindings(registryCoordinatorAddr, operatorStateRetrieverAddr gethcommon.Address, ethclient eth.Client, logger logging.Logger) (*AvsManagersBindings, error) {
	...
	taskManagerAddr, err := contractServiceManager.CrossMessagingTaskManager(&bind.CallOpts{})
	if err != nil {
	...
	if err != nil {
		logger.Error("Failed to fetch ICrossMessagingTaskManager contract", "err", err)
		return nil, err
	}
